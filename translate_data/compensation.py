...
#def separation_tax(data_line):  # для создания транзакция по взаимных расчетов налогов при использовании общего счета
#    owner_res = None
    #  dl = dt.list_date_tax
...
#    if (dl[0] == owner) and (dl[1] < date) and (date < dl[2]) and (status is dl[7]):
#        summa = data_line.summa  # получение суммы для дальнейших расчетов
#        summa_a = summa * dl[3]  # деление суммы на части согласно пропорции
#        owner_res = dl[4]  # получение на какого владельца платежки изменить изначальную
...
#        data_line1.owner = dl[5]  # получение на какого владельца платежки изменить согласно list_date_owner
#        data_line2.owner = dl[6]  # получение на какого владельца платежки изменить согласно list_date_owner
...
def between_themselves(block, date_max):
